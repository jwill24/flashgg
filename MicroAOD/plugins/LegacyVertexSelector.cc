...
namespace flashgg {
    ...
    public:
        ...
        edm::Ptr<reco::Vertex> select( const edm::Ptr<flashgg::Photon> &, const edm::Ptr<reco::CompositeCandidate> &, const std::vector<edm::Ptr<reco::Vertex> > &,
                                       const VertexCandidateMap &vertexCandidateMap,
                                       const std::vector<edm::Ptr<reco::Conversion> > &,
                                       const std::vector<edm::Ptr<reco::Conversion> > &,
                                       const math::XYZPoint &,
    ...
    edm::Ptr<reco::Vertex> LegacyVertexSelector::select( const edm::Ptr<flashgg::Photon> &pho, const edm::Ptr<reco::CompositeCandidate> &cc,
                                                         const std::vector<edm::Ptr<reco::Vertex> > &vtxs,
                                                         const VertexCandidateMap &vertexCandidateMap,
                                                         const std::vector<edm::Ptr<reco::Conversion> > &conversionsVector,
                                                         const std::vector<edm::Ptr<reco::Conversion> > &conversionsVectorSingleLeg,
                                                         ...
                                                         )
    {
        ...
        int IndexMatchedConversionPhoton = -1;
        std::vector<int> vIndexMatchedConversionPhoton;
        float nConvLegs_Photon = 0;   
        
        if( conversionsVector.size() > 0 ) {
            if( (pho->hasConversionTracks() && !pureGeomConvMatching) || pureGeomConvMatching ) {
                vIndexMatchedConversionPhoton = IndexMatchedConversion( pho, conversionsVector, conversionsVectorSingleLeg, useSingleLeg );
                IndexMatchedConversionPhoton = vIndexMatchedConversionPhoton[0];
                nConvLegs_Photon = vIndexMatchedConversionPhoton[1];
            }
        }
        ...
        for( vertex_index = 0 ; vertex_index < vtxs.size() ; vertex_index++ ) {
            ...
            TVector3 PhotonDir;
            TVector3 PhotonDir_uv;
            TLorentzVector p14;
            PhotonDir.SetXYZ( pho->superCluster()->position().x() - vtx->position().x(),
                              pho->superCluster()->position().y() - vtx->position().y(),
                              pho->superCluster()->position().z() - vtx->position().z() );
            PhotonDir_uv = PhotonDir.Unit() * pho->superCluster()->rawEnergy();
            p14.SetPxPyPzE( PhotonDir_uv.x(), PhotonDir_uv.y(), PhotonDir_uv.z(), pho->superCluster()->rawEnergy() );

            TLorentzVector p24;
            p24.SetPxPyPzE(cc->px(), cc->py(), cc->pz(), cc->energy());
            ...
            float nConv = 0;
            if( IndexMatchedConversionPhoton != -1 ) { ++nConv; }
            ...
            if( nConv != 0 ) {
                ...
                const edm::Ptr<flashgg::Photon> &dummyg2 = pho;

                zconv = zFromConvPair( pho, dummyg2, IndexMatchedConversionPhoton, -1 /*FIXME*/, nConvLegs_Photon, 0 /*FIXME*/,
                                       conversionsVector, conversionsVectorSingleLeg, beamSpot );

                szconv = sZFromConvPair( pho, dummyg2, IndexMatchedConversionPhoton, -1 /*FIXME*/, nConvLegs_Photon, 0 /*FIXME*/,
                                         conversionsVector, conversionsVectorSingleLeg );
                if( szconv != 0 ) {
                    pull_conv = fabs( vtx->position().z() - zconv ) / szconv;
                } else {
